import ctypes, time, subprocess, sys, json, os
...
'''
# 用法: python bench.py moves <baseline.dll> [<new.dll>]
BASE = sys.argv[1] if len(sys.argv) > 1 else './ai_baseline.dll'
NEW  = sys.argv[2] if len(sys.argv) > 2 else '../ai.dll'
HERE = os.path.dirname(os.path.abspath(__file__))
WORKER = os.path.join(HERE, '_one.py')   # 寫在 lib/ 自己旁邊，不依賴 CWD
open(WORKER,'w',encoding='utf-8').write(CODE)
...
for key in ["quiet-8","quiet-12","quiet-16"]:
    r=[]
    for dll in [BASE, NEW]:
        out=subprocess.run([sys.executable,WORKER,dll,key],capture_output=True,text=True,encoding='utf-8')
